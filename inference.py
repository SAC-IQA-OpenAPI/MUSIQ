...
config = Config({
    ...
    'dirname': '/data/test',     # directory of data root
    'checkpoint': './weights/5_35000.pth',                              # weights of trained model
...
for idx in tqdm(range(len(df))):
    ...
    # print(filename)
    # print(d_img_name, " - d_img_name")
    d_img_org = cv2.imread(d_img_name + '.jpg')
    d_img_org = cv2.cvtColor(d_img_org, cv2.COLOR_BGR2RGB)
    d_img_org = cv2.resize(d_img_org, (1024, 768))
    ...
    # f.write(line)
    df.at[idx, 'mos'] = pred[0].item()
    # df.to_csv('/data/prediction.csv')
    # break
df.to_csv('/data/MUSIQ_5epoch_prediction.csv', index=False)
# f.close()
